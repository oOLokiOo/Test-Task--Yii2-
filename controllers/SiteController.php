...
class SiteController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $posted = current($_POST['UserDB']);
            $attribute = Yii::$app->request->post('editableAttribute');

            Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;

            if (UserDB::updateAttributeById($userId, $attribute, $posted[$attribute])) {
                return ['result' => 'Success', 'id' => $userId];
            } else {
                return ['result' => 'Error', 'id' => $userId];
            }
        } elseif (Yii::$app->request->post('hasDeleted')) {
        ...
        return $this->render('login', [
            'model' => $model,
            'dataProvider' =>  UserDB::getDataProvider(),
        ]);
    }
